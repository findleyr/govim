...
func (s *snapshot) PackageHandles(ctx context.Context, fh source.FileHandle) ([]source.PackageHandle, error) {
	...
	stdlog.Printf("snapshot %v PackageHandles for %v: meta %v, shouldCheck %#v %v %v", s.id, fh.Identity().URI, meta0(meta), phs, load, check)
	...
	if load {
		...
		meta = newMeta
		stdlog.Printf("snapshot %v PackageHandles for %v: loaded new meta %v", s.id, fh.Identity().URI, meta0(meta))
	}
	if check {
	...
	stdlog.Printf("snapshot %v PackageHandles for %v: ph %#v", s.id, fh.Identity().URI, phs[0])
...
func (s *snapshot) clone(ctx context.Context, withoutFile source.File) *snapshot {
	...
	stdlog.Printf("invalidate %v: original FH %#v", s.id, originalFH)
	...
	if originalFH == nil {
		for pkgId, md := range s.metadata {
			stdlog.Printf("examining %v\n", md.pkgPath)
			for _, cgf := range md.compiledGoFiles {
				stdlog.Printf("checking %v vs %v\n", dir(withoutFile.URI().Filename()), dir(cgf.Filename()))
				if dir(withoutFile.URI().Filename()) == dir(cgf.Filename()) {
					ids[pkgId] = struct{}{}
				}
			}
		}
		stdlog.Printf("invalidate %v: guessed file IDs %#v", s.id, ids)
	}

	// If there is no known FileHandle and no known IDs for the given file,
	// there is nothing to invalidate.
	if len(ids) == 0 && originalFH == nil {
		stdlog.Printf("invalidate %v: uh oh, cannot invalidate", s.id)
	...
	if s.view.session.cache.shouldLoad(ctx, s, originalFH, currentFH) {
		for id := range ids {
	...
	stdlog.Printf("new snapshot %v has ids %#v", result.id, result.ids)
...
func (v *view) invalidateContent(ctx context.Context, f source.File, action source.FileAction) bool {
	...
	stdlog.Printf("invalidate snapshot %v due to %v with action %v", v.snapshot.id, f.URI(), action)

	newSnap := v.snapshot.clone(ctx, f)
	...
	v.snapshot = newSnap
	stdlog.Printf("invalidate %v with action %v created new snapshot %v.", f.URI(), action, v.snapshot.id)
	return true
}
