...
func (s *snapshot) load(ctx context.Context, scope source.Scope) ([]*metadata, error) {
	...
	stdlog.Printf("snapshot %v: go/packages.Load: %#v", s.id, pkgs)
	if len(pkgs) == 0 {
		if err == nil {
...
func (s *snapshot) updateMetadata(ctx context.Context, uri source.Scope, pkgs []*packages.Package, cfg *packages.Config) ([]*metadata, error) {
	...
	for _, pkg := range pkgs {
		stdlog.Printf("go/packages.Load: %v, %v\n", tag.Of("package", pkg.PkgPath), tag.Of("files", pkg.CompiledGoFiles))
		...
		stdlog.Printf("go/packages.Load: m: %v\n", m)
		if m != nil {
