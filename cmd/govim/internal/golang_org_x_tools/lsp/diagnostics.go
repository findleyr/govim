...
func (s *Server) diagnoseFile(snapshot source.Snapshot, f source.File) {
	...
	stdlog.Printf("generating diagnostics for %v", f.URI())
	...
	if err != nil {
		stdlog.Printf("diagnoseFile: could not generate diagnostics for %v: %v", f.URI(), err)
		return
	}
	stdlog.Printf("diagnoseFile: publishing diagnostics for %v: %#v", f.URI(), reports)
...
func (s *Server) publishReports(ctx context.Context, reports map[source.FileIdentity][]source.Diagnostic, publishEmpty bool) {
	...
	for fileID, diagnostics := range reports {
		...
		if len(diagnostics) == 0 && !publishEmpty {
			stdlog.Printf("publishReports: not publishing for %#v for %v: %v", diagnostics, fileID.URI, publishEmpty)
			continue
		}
